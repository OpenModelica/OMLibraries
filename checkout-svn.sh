...
if ! test -d "$DEST"; then
  svn co $SVNOPTS "-r$REVISION" "$URL" "$DEST" --trust-server-cert --trust-server-cert-failures=unknown-ca,cn-mismatch,expired,not-yet-valid,other --non-interactive || exit 1
  ...
  elif ! svn up $SVNOPTS "-r$REVISION" "$DEST" --trust-server-cert --trust-server-cert-failures=unknown-ca,cn-mismatch,expired,not-yet-valid,other --non-interactive; then
